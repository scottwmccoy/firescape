"""Shared spec and furniture for the urban-corridor sheets.

Two figures render the same two windows -- :mod:`p10_urban_zooms` the
annualized triptych, :mod:`p12_urban_zoom_forecast` the forecast-style pair --
and they have to agree on where the window is, which places and watercourses
get named, and how those labels look. Everything both need lives here, so
moving the Reno bounds or adding a corridor moves both maps at once.
...
def context(key):
    ...
    # is not. The result caches here, so it is a one-time cost per window.
    rv_path = inter / "context_major_rivers.geojson"
    if rv_path.exists():
        ...
        keep = rv["name"].astype(str).apply(
            lambda n: any(w.lower() in n.lower() for w in z["rivers"]))
...
def figure_width(bounds, panels, *, panel_h, cbar=0.95, edge=0.5):
    """Figure width that leaves no slack around ``panels`` map panels.
    ...
    corridor -- tall-narrow Reno floated in a sea of margin. Size the figure to
    ...
    ratio = lon_span * math.cos(mid_lat) / lat_span
    return panels * (panel_h * ratio + cbar) + edge
