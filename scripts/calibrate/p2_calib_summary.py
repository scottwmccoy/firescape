...
OUT_TOML = (paths.package_data("calibration", "statewide_v1.toml"))
...
for _, row in use.iterrows():
    ...
    for table in ("staley2018", "nv_merged"):
...
st = cal[cal["table"] == "staley2018"].groupby("region")["pdsim"].median()
...
lines = [
    ...
    'method = "rossi2025-dfl-calibration (logit-space solve, per-region)"',
...
with open(OUT_TOML, "w") as f:
    f.write("\n".join(lines) + "\n")
print(f"\nwrote {OUT_TOML}")
cal.to_csv(paths.products_dir("calibration") / "statewide_v1_fires.csv", index=False)
