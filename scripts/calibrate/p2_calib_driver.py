"""Statewide per-region P_dsim calibration driver (resumable, exit 42).
...
for _, row in use.sort_values("km2", ascending=False).iterrows():
    ...
    tag = f"{vintage.split('_')[0].lower()}_b{brk:.0f}"
    if calibrate.load_cached(eid, tag, table="nv_merged") is not None:
        done.append(eid)
        continue
    ...
    try:
        ...
        res = calibrate.fire_calibration(
            ...
            evt=evt, crosswalk=mapping, dem=dem, cdf_tables=TABLES, tag=tag)
        fc = res["nv_merged"]
