...
# Ascending: every fire with a cached decomposition re-solves in ~2 s, so bank
# those first. MARTIN (1,749 km2) was calibrated before decomposition caching
# existed and is the one fire still needing the full raster path -- it sorts
# last, and this run backfills its cache.
for _, row in use.sort_values("km2", ascending=True).iterrows():
    eid = row["event_id"]
    vintage = row["evt_vintage"]
    brk = float(breaks[row["region"]])
    tag = f"{vintage.split('_')[0].lower()}_b{brk:.0f}_d91sw"
    ...
    if time.time() - T0 > BUDGET:
        ...
        continue
    # The class decomposition is independent of the CDF table, so a sibling
    # cache turns this into a pure re-solve -- and then the DEM and EVT
    # rasters are never read. Staging them regardless costs minutes per large
    # fire for nothing (12 min on Roosters Comb before this check existed).
    cached = calibrate._find_decomp(eid, tag) is not None
