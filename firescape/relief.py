...
1. **Warp each tile exactly once, from its native grid, with bilinear.** No
   ...
   same error class stormscape removed on 2026-07-31 (see its CLAUDE.md: a
   nearest warp inside ``py3dep.get_dem`` was biasing slope ~1.8 deg and
   halving plan curvature). ``statewide.dem_for_unit`` already follows this
   for the science grids; so does this module for the display grid.
...
2. **Mosaic elevations into one continuous surface before shading.** Shading
   ...
   reason, mirroring stormscape's "+20 cells so the kernel never reaches past
   the data".
...
3. **Shade finer than the display grid, then average the shaded array down.**
   ...
   mirrors stormscape's ``plot._prepare_hillshade``, which downsamples the
   *hillshade* with ``Resampling.average``.
...
#: Sun position and vertical exaggeration (matches stormscape's dem.hillshade).
AZIMUTH_DEG = 315.0
ALTITUDE_DEG = 45.0
VERT_EXAG = 1.5
...
def _elevation_mosaic(tiles, bounds5070, res_m: float):
    ...
    Kept separate from :func:`statewide._mosaic_to_grid`, which serves the
    science grids: that one snaps/pads a unit-sized grid and also handles
