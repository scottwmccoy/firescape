...
def inset_colorbar(ax, mappable, label, *, corner="lower left", pad=0.018,
    ...
    House style for any multi-panel sheet whose panels are aspect-locked and
    meant to be compared: promoted here from p15 so the fire sheets and the
    hindcast sheets share one implementation rather than drifting apart.
    ...
    x, y = corner_xy(corner, (w, h), pad)
    ax.add_patch(_Rectangle((x, y), w, h, transform=ax.transAxes,
    ...
    cb.locator = _MaxNLocator(nbins=nticks - 1)
    cb.update_ticks()
    if twin is None:
